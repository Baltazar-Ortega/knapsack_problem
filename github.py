...
def solution():
    ...
    for i in range(1,variables):
        ...
        # x_1, x_2
        variableslist.append(LpVariable('x_{no}'.format(no=i),0)) 
    ...
    tree.create_node('Original','original',
    data={'status':LpStatus[prob.status],
        'problem':prob,
        'non_int':non_int_variables,
        'result':Result}) # First node is the original problem
    
    p = 0
    
    while node_list: # Mientras la lista tenga elementos
